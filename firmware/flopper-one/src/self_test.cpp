...
namespace flopper
{
    ...
    void SelfTest::runI2CScan()
    {
        ...
        if (found == 0)
            Serial.println("[SELFTEST][I2C] no devices found");
    }
    ...
    {
        const uint32_t now = millis();
        if (now - lastDisplayMs_ < 1000)
            return;
        lastDisplayMs_ = now;

        static bool inited = false;
        auto &disp = flopper::Display::get_instance();
        if (!inited)
        {
            disp.clear(flopper::colors::BLACK);
            disp.draw_text(10, 10, "SELF TEST", flopper::colors::GREEN, 2, flopper::colors::BLACK);
            inited = true;
            Serial.println("[SELFTEST][DISP] Adafruit_GFX display initialized");
        }

        disp.fill_rect(10, 40, 220, 30, flopper::colors::BLACK);
        char buf[32];
        snprintf(buf, sizeof(buf), "ms %lu", (unsigned long)now);
        disp.draw_text(10, 40, buf, flopper::colors::GREEN, 2, flopper::colors::BLACK);
    }
